...
conda run -n dl python infer_clip.py all-training/clip_checkpoints/step_12000.pt \
...
- **`aux` loss near 0 from the start** — `f_init_bias` (computed on the fly from `L`, `d_sine`, and the row's real length when a pad mask is given) already spreads initial frequencies across the band, so most pairs start far apart. Not a bug.
...
- **Checkpoints store the full `cfg` dict.** `infer.py` and `infer_clip.py` both rehydrate via `Config(**blob["cfg"])`, so checkpoints from older configs are loadable as long as the *parameter shapes* haven't changed. AE checkpoints (`train.py`) and CLIP checkpoints (`train_clip.py`) share the model state-dict format but CLIP checkpoints additionally store `logit_scale`. Old FFT-era checkpoints will NOT load (architecture changed). Checkpoints across different `d_sine` will NOT load each other (encoder head + decoder input proj shapes change).
